      ...
      totalesBtn.addEventListener('click', (ev) => {
        ...
        const makeTotalsBtn = (text, idSuffix) => {
          ...
          b.addEventListener('click', (ev) => {
            ev.stopPropagation();
            // Remove initial "Seleccionar un total" prompt as soon as any quick totals button is pressed
            try {
              const msgEl = document.getElementById('totales_select_message');
              if(msgEl) msgEl.remove();
            } catch (_) {}
            // Always remove any previously rendered Totales/asistencia panels before creating a new one
            try {
              const removeIds = ['asistenciaPanels','totales_publicadores_panel','totales_prec_aux_panel'];
              removeIds.forEach(id => {
                // prefer removing from the Totales card if present, otherwise remove from document
                try {
                  const inCard = card && card.querySelector && card.querySelector('#' + id);
                  if(inCard) inCard.remove();
                  else {
                    const elsewhere = document.getElementById(id);
                    if(elsewhere) elsewhere.remove();
                  }
                } catch(e){}
              });
            } catch(e){}
            try {
              ...
              if(idSuffix === 'asistencia'){
                // Remove any previous asistencia/publicadores/prec_aux panels to avoid duplicates stacking
                try {
                  ...
                  if (existingPub) existingPub.remove();
                  const existingPrecAux = card.querySelector('#totales_prec_aux_panel');
                  if (existingPrecAux) existingPrecAux.remove();
                  const existingPrecReg = card.querySelector('#totales_prec_reg_panel');
                  if (existingPrecReg) existingPrecReg.remove();
                } catch (e) { /* ignore */ }
                ...
                const _now = new Date();
                let serviceYearDefault = (function(){
                  try{
                    const stored = localStorage.getItem('totales_service_year_asistencia_v1');
                    if(stored && /^\d{4}$/.test(stored)) return Number(stored);
                  }catch(e){}
                  return (_now.getMonth() >= 8) ? (_now.getFullYear() + 1) : _now.getFullYear();
                })();
                ...
                function setServiceYearAndRebuild(y){
                  ...
                  try{ localStorage.setItem('totales_service_year_asistencia_v1', String(serviceYearDefault)); }catch(e){}
              ...
              if(idSuffix === 'publicadores'){
                // Build a "Totales Publicadores" panel with 3 columns and 12 service-year months
                // remove any previous content area message
                try {
                  const existingPanels = card.querySelector('#totales_publicadores_panel');
                  if(existingPanels) existingPanels.remove();
                } catch(e){}

                // create panel container
                const pubPanel = document.createElement('div');
                pubPanel.id = 'totales_publicadores_panel';
                pubPanel.style.width = '100%';
                pubPanel.style.background = '#fff';
                pubPanel.style.border = '1px solid var(--subtle)';
                pubPanel.style.borderRadius = '8px';
                // make panel padding tighter so the table sits a bit higher (more compact vertically)
                pubPanel.style.padding = '8px 12px 10px 12px';
                pubPanel.style.boxSizing = 'border-box';

                const hdr = document.createElement('div');
                hdr.style.display = 'flex';
                hdr.style.justifyContent = 'space-between';
                hdr.style.alignItems = 'center';
                // reduce header bottom spacing to pull table upward
                hdr.style.marginBottom = '6px';

                const title = document.createElement('div');
                title.textContent = 'Totales Publicadores';
                title.style.fontWeight = '800';
                title.style.fontSize = '15px';
                hdr.appendChild(title);

                pubPanel.appendChild(hdr);

                // create table (slightly smaller font to compact rows)
                const table = document.createElement('table');
                table.style.width = '100%';
                table.style.borderCollapse = 'collapse';
                table.style.tableLayout = 'fixed';
                ...
                ['Mes','N° Informes','Estudios Bíblicos'].forEach((c, i) => {
                  const th = document.createElement('th');
                  th.textContent = c;
                  th.style.padding = '8px';
                  th.style.borderBottom = '1px solid var(--subtle)';
                ...
                // compute service-year months (Sept prev year -> Aug year)
                const now = new Date();
                const defaultServiceYear = (now.getMonth() >= 8) ? (now.getFullYear() + 1) : now.getFullYear();
                const months = [];
                const start = new Date(defaultServiceYear - 1, 8, 1); // Sept prev year
                ...
                // For each month create a row: N° Informes = count of publicadores (Hours === 1) and Estudios Bíblicos = sum of their studies;
                // only compute values for months that are "mes con registro", otherwise show zeros.
                months.forEach(mo => {
                  const tr = document.createElement('tr');
                  tr.style.borderBottom = '1px solid rgba(15,23,42,0.03)';
                  ...
                  tdCourses.style.padding = '8px';
                  tdCourses.style.textAlign = 'center';

                  try {
                    // if the month has no global records, show zeros
                    if(!monthHasRecordsGlobal(mo.key)){
                      ...
                      tdNum.textContent = String(informesCount);
                      tdCourses.textContent = String(studiesSum);
                    }
                  } catch (err) {
                    tdNum.textContent = '0';
                    tdCourses.textContent = '0';
                ...
                // Add Totals and Average rows: compute sums and averages (average over months that have registro)
                try {
                  // compute totals by iterating tbodyLocal rows
                  let totalInformes = 0;
                  let totalStudies = 0;
                  Array.from(tbodyLocal.querySelectorAll('tr')).forEach(r => {
                    try {
                      const numTd = r.children[1];
                      const coursesTd = r.children[2];
                      const num = numTd ? Number((numTd.textContent || '').trim()) : 0;
                      const courses = coursesTd ? Number((coursesTd.textContent || '').trim()) : 0;
                      if(!isNaN(num)) totalInformes += num;
                      if(!isNaN(courses)) totalStudies += courses;
                    } catch(e){}
                  });

                  // determine number of months that have registro (use monthHasRecordsGlobal defined above)
                  let monthsWithRegistroCount = 0;
                  try {
                    months.forEach(mo => {
                      if(typeof monthHasRecordsGlobal === 'function' && monthHasRecordsGlobal(mo.key)) monthsWithRegistroCount++;
                    });
                  } catch (e){ monthsWithRegistroCount = 0; }

                  // Totals row
                  const trTotals = document.createElement('tr');
                  trTotals.style.fontWeight = '700';
                  trTotals.style.borderTop = '2px solid rgba(15,23,42,0.06)';
                  const tdLabelT = document.createElement('td');
                  tdLabelT.style.padding = '8px';
                  tdLabelT.textContent = 'Totales';
                  const tdTotalInformes = document.createElement('td');
                  tdTotalInformes.style.padding = '8px';
                  tdTotalInformes.style.textAlign = 'center';
                  tdTotalInformes.textContent = String(totalInformes);
                  const tdTotalCourses = document.createElement('td');
                  tdTotalCourses.style.padding = '8px';
                  tdTotalCourses.style.textAlign = 'center';
                  tdTotalCourses.textContent = String(totalStudies);
                  trTotals.appendChild(tdLabelT);
                  trTotals.appendChild(tdTotalInformes);
                  trTotals.appendChild(tdTotalCourses);
                  tbodyLocal.appendChild(trTotals);

                  // Average row (average based only on months that have registro)
                  const trAvg = document.createElement('tr');
                  trAvg.style.fontWeight = '700';
                  const tdLabelA = document.createElement('td');
                  tdLabelA.style.padding = '8px';
                  tdLabelA.textContent = 'Promedio';
                  const tdAvgInformes = document.createElement('td');
                  tdAvgInformes.style.padding = '8px';
                  tdAvgInformes.style.textAlign = 'center';
                  const tdAvgCourses = document.createElement('td');
                  tdAvgCourses.style.padding = '8px';
                  tdAvgCourses.style.textAlign = 'center';
                  const denom = monthsWithRegistroCount > 0 ? monthsWithRegistroCount : 1;
                  tdAvgInformes.textContent = String(Math.round(totalInformes / denom));
                  tdAvgCourses.textContent = String(Math.round(totalStudies / denom));
                  trAvg.appendChild(tdLabelA);
                  trAvg.appendChild(tdAvgInformes);
                  trAvg.appendChild(tdAvgCourses);
                  tbodyLocal.appendChild(trAvg);
                } catch (err) {
                  // if any error computing totals/averages, still append table without these rows
                  console.warn('Error computing Totales Publicadores totals/averages', err);
                }

                pubPanel.appendChild(table);



                // function to recompute Pub. totals using selected service year
                function refreshPubPanelForYear(year){
                  try {
                    const months = monthsForServiceYear_pub(year);
                    // recompute totals and update rows in tbodyLocal if present
                    const tbodyLocalRef = pubPanel.querySelector('tbody');
                    if(!tbodyLocalRef) return;
                    // iterate rows (first 12 rows correspond to months in chronological order)
                    const rows = Array.from(tbodyLocalRef.querySelectorAll('tr'));
                    let totalInformes = 0;
                    let totalStudies = 0;
                    let monthsWithRegistroCount = 0;
                    // helper: whether month has registro globally (at least one person with hours >=1)
                    function monthHasRecordsGlobalPub(monthKey){
                      if(!monthKey) return false;
                      for(const p of people){
                        if(!p.activities) continue;
                        const act = p.activities[monthKey];
                    ...
                    months.forEach((mk, idx) => {
                      const row = rows[idx];
                      if(!row) return;
                      const numTd = row.children[1];
                      const coursesTd = row.children[2];
                      try {
                        if(!monthHasRecordsGlobalPub(mk)){
                          numTd.textContent = '0';
                          coursesTd.textContent = '0';
                        } else {
                          let informesCount = 0;
                          let studiesSum = 0;
                          people.forEach(p => {
                          ...
                          numTd.textContent = String(informesCount);
                          coursesTd.textContent = String(studiesSum);
                        }
                      } catch (err){
                        numTd.textContent = '0';
                        coursesTd.textContent = '0';
                    ...
                    // recompute totals/averages below tbodyLocal (Totales & Promedio rows are last two appended)
                    try {
                      // gather numeric values from tbody rows (excluding last two footer rows if present)
                      const dataRows = Array.from(tbodyLocalRef.querySelectorAll('tr')).filter(r => r.querySelectorAll('td').length === 3);
                      totalInformes = 0;
                      totalStudies = 0;
                      monthsWithRegistroCount = 0;
                      months.forEach((mk, idx) => {
                        if(!monthHasRecordsGlobalPub(mk)) return;
                        const r = dataRows[idx];
                        if(!r) return;
                        const n = Number((r.children[1] && r.children[1].textContent) ? r.children[1].textContent : 0) || 0;
                        const s = Number((r.children[2] && r.children[2].textContent) ? r.children[2].textContent : 0) || 0;
                        totalInformes += n;
                        totalStudies += s;
                        monthsWithRegistroCount++;
                      });

                      // If footer rows exist (Totales and Promedio), update them
                      const footerTotalsRow = Array.from(tbodyLocalRef.querySelectorAll('tr')).find(r => (r.children[0] && r.children[0].textContent === 'Totales'));
                      const footerAvgRow = Array.from(tbodyLocalRef.querySelectorAll('tr')).find(r => (r.children[0] && r.children[0].textContent === 'Promedio'));
                      if(footerTotalsRow){
                        if(footerTotalsRow.children[1]) footerTotalsRow.children[1].textContent = String(totalInformes);
                        if(footerTotalsRow.children[2]) footerTotalsRow.children[2].textContent = String(totalStudies);
                      }
                      if(footerAvgRow){
                        const denom = monthsWithRegistroCount > 0 ? monthsWithRegistroCount : 1;
                        if(footerAvgRow.children[1]) footerAvgRow.children[1].textContent = String(Math.round(totalInformes / denom));
                        if(footerAvgRow.children[2]) footerAvgRow.children[2].textContent = String(Math.round(totalStudies / denom));
                      }
                    } catch(e){}
                  } catch(e){}
                }
                ...
                // insert or replace into the Totales card body area (bodyArea exists earlier)
                ...
                const msgEl = document.getElementById('totales_select_message');
                if(msgEl) msgEl.remove();
                // remove any previous panels for asistencia/publicadores etc.
                const existingAsis = card.querySelector('#asistenciaPanels');
                if(existingAsis) existingAsis.remove();
                const existingPub = card.querySelector('#totales_publicadores_panel');
                if(existingPub) existingPub.remove();

                // Remove any leftover "Año de servicio" wrappers from prior Totales views
                // (prevents the Asistencia year indicator from remaining above the Publicadores table)
                try {
                  Array.from(card.querySelectorAll('[id$="_year_wrap"]')).forEach(el => {
                ...
                if(bodyArea){
                  ...
                  bodyArea.insertBefore(pubPanel, bodyArea.firstChild || null);
                } else {
                  card.appendChild(pubPanel);
                }

              } else if(idSuffix === 'prec_aux'){
                // Build a "Totales Prec. Auxiliares" panel showing 12 service-year months with four columns:
                // "Mes", "N° Informes", "Horas", "Estudios Bíblicos"
                try {
                  // remove any previous panel
                  const prev = card.querySelector('#totales_prec_aux_panel');
                  if(prev) prev.remove();

                  const panel = document.createElement('div');
                  panel.id = 'totales_prec_aux_panel';
                  panel.style.width = '100%';
                  panel.style.background = '#fff';
                  panel.style.border = '1px solid var(--subtle)';
                  panel.style.borderRadius = '8px';
                  // make panel slightly more compact vertically
                  panel.style.padding = '8px 12px 10px 12px';
                  ...
                  hdr.style.marginBottom = '6px';
                  const title = document.createElement('div');
                  title.textContent = 'Totales Prec. Auxiliares';
                  title.style.fontWeight = '800';
                  title.style.fontSize = '15px';
                  hdr.appendChild(title);
                  ...
                  const thead = document.createElement('thead');
                  const thr = document.createElement('tr');
                  ['Mes','N° Informes','Horas','Estudios Bíblicos'].forEach((c, i) => {
                    const th = document.createElement('th');
                    th.textContent = c;
                    th.style.padding = '8px';
                    th.style.borderBottom = '1px solid var(--subtle)';
                  ...
                  // For each month build row: count aux=true, sum hours (numeric), sum studies (numeric)
                  months.forEach(mo => {
                    const tr = document.createElement('tr');
                    tr.style.borderBottom = '1px solid rgba(15,23,42,0.03)';
                    ...
                    try {
                      ...
                      people.forEach(p => {
                        ...
                        const act = p.activities[mo.key];
                        if(!act) return;
                        // count as informe when aux is true for that month
                        if(!!act.aux){
                          informesCount++;
                          const hStr = (act.hours !== undefined && act.hours !== null) ? String(act.hours).trim() : '';
                          const hs = Number(hStr);
                          if(hStr !== '' && !isNaN(hs)) hoursSum += hs;
                          const sStr = (act.studies !== undefined && act.studies !== null) ? String(act.studies).trim() : '';
                          const ss = Number(sStr);
                          if(sStr !== '' && !isNaN(ss)) studiesSum += ss;
                        }
                      });

                      tdNum.textContent = String(informesCount);
                      tdHours.textContent = String(hoursSum);
                      tdCourses.textContent = String(studiesSum);
                    } catch (err) {
                      tdNum.textContent = '0';
                  ...
                  // Totals & Promedio rows: compute sums and averages over months that have at least one informe
                  try {
                    let totalInformes = 0;
                    let totalHours = 0;
                    let totalStudies = 0;
                    ...
                    tdTotalHours.style.padding = '8px';
                    tdTotalHours.style.textAlign = 'center';
                    tdTotalHours.textContent = String(totalHours);
                    const tdTotalStudies = document.createElement('td');
                    tdTotalStudies.style.padding = '8px';
                    tdTotalStudies.style.textAlign = 'center';
                    tdTotalStudies.textContent = String(totalStudies);
                    trTotals.appendChild(tdLabelT);
                    trTotals.appendChild(tdTotalInformes);
                    trTotals.appendChild(tdTotalHours);
                    trTotals.appendChild(tdTotalStudies);
                    tbodyLocal.appendChild(trTotals);
                    ...
                    tdAvgInformes.style.padding = '8px';
                    tdAvgInformes.style.textAlign = 'center';
                    const tdAvgHours = document.createElement('td');
                    tdAvgHours.style.padding = '8px';
                    tdAvgHours.style.textAlign = 'center';
                    const tdAvgStudies = document.createElement('td');
                    tdAvgStudies.style.padding = '8px';
                    tdAvgStudies.style.textAlign = 'center';
                    const denom = monthsWithInforme > 0 ? monthsWithInforme : 1;
                    tdAvgInformes.textContent = String(Math.round(totalInformes / denom));
                    tdAvgHours.textContent = String(Math.round(totalHours / denom));
                    tdAvgStudies.textContent = String(Math.round(totalStudies / denom));
                    trAvg.appendChild(tdLabelA);
                    trAvg.appendChild(tdAvgInformes);
                    trAvg.appendChild(tdAvgHours);
                    trAvg.appendChild(tdAvgStudies);
                    tbodyLocal.appendChild(trAvg);
                  } catch (err) {
                    console.warn('Error computing Totales Prec. Auxiliares totals/averages', err);
                  }
                  ...
                  console.error('Error building Totales Prec. Auxiliares panel', err);
                ...
                try {
                  ...
                  const panel = document.createElement('div');
                  panel.id = 'totales_prec_reg_panel';
                  panel.style.width = '100%';
                  panel.style.background = '#fff';
                  panel.style.border = '1px solid var(--subtle)';
                  panel.style.borderRadius = '8px';
                  ...
                  hdr.style.marginBottom = '6px';
                  const title = document.createElement('div');
                  title.textContent = 'Totales Prec. Regulares';
                  title.style.fontWeight = '800';
                  title.style.fontSize = '15px';
                  hdr.appendChild(title);
                  panel.appendChild(hdr);
                  ...
                  const thead = document.createElement('thead');
                  const thr = document.createElement('tr');
                  ['Mes','N° Informes','Horas','Estudios Bíblicos'].forEach((c) => {
                    const th = document.createElement('th');
                    th.textContent = c;
                    th.style.padding = '8px';
                    th.style.borderBottom = '1px solid var(--subtle)';
                  ...
                  try {
                    ...
                    console.warn('Error computing Totales Prec. Regulares totals/averages', err);
                  }
                  ...
                  console.error('Error building Totales Prec. Regulares panel', err);
                  alert('Error construyendo Totales Prec. Regulares (ver consola).');
                }
              } else if (idSuffix === 'congregacion') {
                // Totales Congregación: muestra el total simple de personas en el registro
                try {
                  const total = people.length;
                  alert(`Totales Congregación: ${total} persona(s)`);
                } catch (err) {
                  console.error('Error calculando Totales Congregación', err);
                  alert('Error calculando Totales Congregación.');
                }
              }
            } catch (err) {
        ...
        // Nuevo botón: Totales Congregación
        totalsButtonsRow.appendChild(makeTotalsBtn('Totales Congregación','congregacion'));
        ...
        // allow content to expand without creating an internal vertical scrollbar;
        // make overflow visible so inner tables/panels render inside the card and the card itself scrolls if needed
        bodyArea.style.overflow = 'visible';
        bodyArea.style.overflowY = 'visible';
        bodyArea.style.maxHeight = 'none';
        ...
        // Initial centered bold message prompting user to select a total
        const selectMsg = document.createElement('div');
        selectMsg.id = 'totales_select_message';
        selectMsg.style.width = '100%';
        selectMsg.style.display = 'flex';
        selectMsg.style.alignItems = 'center';
        selectMsg.style.justifyContent = 'center';
        selectMsg.style.padding = '40px 0';
        selectMsg.style.fontWeight = '800';
        selectMsg.style.fontSize = '16px';
        selectMsg.style.color = 'var(--text)';
        selectMsg.textContent = 'Seleccionar un total';
        bodyArea.appendChild(selectMsg);
        ...
        // Footer (kept for visual separation; no action button to maximize usable space)
        const footer = document.createElement('div');
        footer.style.display = 'flex';
        footer.style.justifyContent = 'flex-end';
        ...
        // intentionally no close button here to preserve space for indicators/content
        card.appendChild(footer);
...
importFile.addEventListener('change', (e) => {
  ...
  reader.onload = () => {
    try{
      ...
      // If the imported file includes asistencia_values (modern backups), restore them into localStorage
      try {
        if(parsed && parsed.asistencia_values && typeof parsed.asistencia_values === 'object') {
          Object.keys(parsed.asistencia_values).forEach(k => {
            try {
              const v = parsed.asistencia_values[k];
              if(typeof v === 'string') localStorage.setItem(k, v);
              else localStorage.setItem(k, JSON.stringify(v));
            } catch(e){
              console.warn('No se pudo restaurar asistencia key', k, e);
            }
          });
        }
      } catch(e){
        console.warn('Error restaurando asistencia_values durante importación', e);
      }
...
function restoreAppState(state) {
  ...
  try {
    ...
    // Restore people array if present
    ...
    // Restore asistencia_values entries into localStorage if present in backup
    // state.asistencia_values is expected to be an object mapping storageKey -> value (array or string)
    try {
      if(state.asistencia_values && typeof state.asistencia_values === 'object') {
        Object.keys(state.asistencia_values).forEach(k => {
          try {
            const v = state.asistencia_values[k];
            // if value is already a string, persist as-is; otherwise stringify
            if(typeof v === 'string') localStorage.setItem(k, v);
            else localStorage.setItem(k, JSON.stringify(v));
          } catch(e){
            console.warn('No se pudo restaurar key', k, e);
          }
        });
      }
    } catch(e){
      console.warn('Error restaurando asistencia_values desde respaldo', e);
    }
    ...
    // Restore banner title if included
    try {
      if(state.bannerTitle) {
        saveBannerTitle(state.bannerTitle);
        const b = document.querySelector('.banner-title');
        if(b) b.textContent = state.bannerTitle;
      }
    } catch(e){}

    // Restore small meta keys if provided
    try {
      if(state.meta && typeof state.meta === 'object'){
        if(state.meta.jwReportLastMonth) localStorage.setItem('jw_report_last_month_v1', state.meta.jwReportLastMonth);
        if(state.meta.asistenciaLastMonth) localStorage.setItem('asistencia_last_month_v1', state.meta.asistenciaLastMonth);
        if(state.meta.lastLoadedBackup) localStorage.setItem('last_loaded_backup_v1', state.meta.lastLoadedBackup);
      }
    } catch(e){}
    ...
    console.error("Error al restaurar el estado de la aplicación", err);
